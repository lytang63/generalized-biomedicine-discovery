...
from config import fitzpatrick17k_dataroot
...
def get_fitzpatrick17k_datasets(
    train_transform,
    test_transform,
    train_classes,
    ...
    train_csv_name="fitzpatrick17k_label_25percent.csv",
    test_csv_name=None
):

    train_csv_path = os.path.join(fitzpatrick17k_dataroot, train_csv_name)
    test_csv_path = train_csv_path if test_csv_name is None \
        else os.path.join(fitzpatrick17k_dataroot, test_csv_name)

    # full dataset
    full_dataset = Fitzpatrick17kDataset(
        root=fitzpatrick17k_dataroot,
