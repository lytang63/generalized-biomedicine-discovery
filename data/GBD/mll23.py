import os
import numpy as np
import pandas as pd
from copy import deepcopy
from PIL import Image

from torch.utils.data import Dataset

from data.data_utils import subsample_instances
from config import mll23_dataroot


class MLL23Dataset18(Dataset):
    """
    CSV-driven dataset for MLL23.
    Must return: img, label, uq_idx
    CSV columns supported:
      path/img_path/rel_path , and label/label_id/class_id/classId
    """

    def __init__(self, root, csv_path, transform=None):
        self.root = root
        self.transform = transform

        df = pd.read_csv(csv_path)

        # path col
        if "path" in df.columns:
            path_col = "path"
        elif "img_path" in df.columns:
            path_col = "img_path"
        elif "rel_path" in df.columns:
            path_col = "rel_path"
        else:
            raise ValueError(f"CSV must have path/img_path/rel_path. Got: {df.columns.tolist()}")

        # label col
        if "label" in df.columns:
            label_col = "label"
        elif "label_id" in df.columns:
            label_col = "label_id"
        elif "class_id" in df.columns:
            label_col = "class_id"
        elif "classId" in df.columns:
            label_col = "classId"
        else:
            raise ValueError(f"CSV must have label/label_id/class_id. Got: {df.columns.tolist()}")

        self.samples = []
        for p, y in zip(df[path_col].tolist(), df[label_col].tolist()):
            p = str(p)
            y = int(y)

            cand1 = os.path.join(self.root, p)
            if os.path.exists(cand1):
                abs_path = cand1
            else:
                # fallback: root/images/<...>
                abs_path = os.path.join(self.root, "images", p)

            self.samples.append([abs_path, y])

        self.targets = [int(x[1]) for x in self.samples]
        self.uq_idxs = np.array(range(len(self)))

        self.target_transform = None

    def __len__(self):
        return len(self.samples)

    def __getitem__(self, idx):
        img_path, label = self.samples[idx]
        uq_idx = int(self.uq_idxs[idx])

        try:
            img = Image.open(img_path).convert("RGB")
        except Exception as e:
            raise RuntimeError(f"Failed to open image: {img_path} ({type(e).__name__}: {e})")

        if self.transform is not None:
            img = self.transform(img)

        if self.target_transform is not None:
            label = self.target_transform(int(label))

        return img, int(label), uq_idx


def subsample_dataset(dataset, idxs):
    mask = np.zeros(len(dataset)).astype("bool")
    mask[idxs] = True

    dataset.samples = np.array(dataset.samples, dtype=object)[mask].tolist()
    dataset.targets = np.array(dataset.targets, dtype=object)[mask].tolist()
    dataset.uq_idxs = dataset.uq_idxs[mask]

    dataset.samples = [[x[0], int(x[1])] for x in dataset.samples]
    dataset.targets = [int(x) for x in dataset.targets]
    return dataset


def subsample_classes(dataset, include_classes):
    include_classes = list(include_classes)
    keep = [i for i, l in enumerate(dataset.targets) if int(l) in set(include_classes)]
    dataset = subsample_dataset(dataset, keep)
    return dataset


def get_mll23_datasets(
        train_transform,
        test_transform,
        train_classes,
        prop_train_labels=0.5,
        seed=0,
        split_train_val=False,
        val_instances_per_class=5,
        train_csv_name="mll23_label_4k.csv",
        test_csv_name=None,
):
    """
    Same interface style as gastrovision/histoset dataset loaders.
    Returns dict with keys: train_labelled, train_unlabelled, val, test
    """
    np.random.seed(seed)

    train_csv_path = os.path.join(mll23_dataroot, train_csv_name)
    if test_csv_name is None:
        test_csv_path = train_csv_path
    else:
        test_csv_path = os.path.join(mll23_dataroot, test_csv_name)

    train_dataset = MLL23Dataset18(
        root=mll23_dataroot,
        csv_path=train_csv_path,
        transform=train_transform
    )

    # labelled = known classes + subsample by prop_train_labels
    train_dataset_labelled = subsample_classes(deepcopy(train_dataset), include_classes=train_classes)
    subsample_indices = subsample_instances(train_dataset_labelled, prop_indices_to_subsample=prop_train_labels)
    train_dataset_labelled = subsample_dataset(train_dataset_labelled, subsample_indices)

    # unlabelled = complement indices from pool
    unlabelled_indices = set(train_dataset.uq_idxs) - set(train_dataset_labelled.uq_idxs)
    train_dataset_unlabelled = subsample_dataset(deepcopy(train_dataset), np.array(list(unlabelled_indices)))

    test_dataset = MLL23Dataset18(
        root=mll23_dataroot,
        csv_path=test_csv_path,
        transform=test_transform
    )

    all_datasets = {
        "train_labelled": train_dataset_labelled,
        "train_unlabelled": train_dataset_unlabelled,
        "val": None,
        "test": test_dataset,
    }
    return all_datasets