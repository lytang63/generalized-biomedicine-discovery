...
from models.open_clip import create_model_and_transforms,create_model

# sys.path.append("/path/to/GBD/SelEx")  # Add if needed
dino_v1 = False # TLY ori =True
...
from config import exp_root, dino_pretrain_path, dino_pretrain_path2, mmkd_pretrain_path
...
def _compute_accs_for_eval_funcs(y_true, y_pred, mask, args, save_name, T=None, writer=None, print_output=False):
    """Compute (all/old/new) accuracies for each eval func listed in args.eval_funcs.

    Returns:
        accs_by_eval: dict[str, dict[str, float]]  # keys: eval_func -> {all, old, new}
    """
    accs_by_eval = {}
    for ef in getattr(args, "eval_funcs", ["v1"]):
        try:
            all_acc, old_acc, new_acc = log_accs_from_preds(
                ...
                T=T,
                eval_funcs=[ef],
                save_name=f"{save_name}/{ef}",
                print_output=print_output,
                writer=writer,
            )
            accs_by_eval[ef] = {"all": float(all_acc), "old": float(old_acc), "new": float(new_acc)}
        except TypeError:
            # Backward-compat if util signature differs (e.g., no writer / print_output)
            all_acc, old_acc, new_acc = log_accs_from_preds(
                y_true=y_true,
                y_pred=y_pred,
                mask=mask,
...
def _wandb_safe_log(args, payload, step=None):
...
def train( projection_head, model, train_loader, test_loader, unlabelled_train_loader, merge_train_loader, args):
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            if (epoch+1) % train_report_interval == 0:
                ...
                all_acc, old_acc, new_acc, accs_train_unl = test_kmeans(model, unlabelled_train_loader, epoch=epoch, save_name='Train ACC Unlabelled',args=args)
            else:
                all_acc, old_acc, new_acc = metrics["all_acc"], metrics["old_acc"], metrics["new_acc"]
                accs_train_unl = metrics.get("accs_by_eval", {})
            print('Testing on disjoint test set...')
            all_acc_test, old_acc_test, new_acc_test, accs_test = test_kmeans(model, test_loader, epoch=epoch, save_name='Test ACC', args=args)
        ...
        _wandb_safe_log(args, {
            "train/loss_total": float(loss_record.avg),
            "train/loss_contrastive_mix": float(loss_cons_record.avg),
            "train/acc_contrastive_logits": float(train_acc_record.avg),
            "train/lr": float(get_mean_lr(optimizer)),
        }, step=epoch)

        # Train (unlabelled split) eval accs by eval func
        if 'accs_train_unl' in locals():
            for ef, m in accs_train_unl.items():
                _wandb_safe_log(args, {
                    f"eval_train_unlabel/{ef}_all": m["all"],
                    f"eval_train_unlabel/{ef}_old": m["old"],
                    f"eval_train_unlabel/{ef}_new": m["new"],
                }, step=epoch)

        # Test eval accs by eval func
        if 'accs_test' in locals():
            for ef, m in accs_test.items():
                _wandb_safe_log(args, {
                    f"eval_test/{ef}_all": m["all"],
                    f"eval_test/{ef}_old": m["old"],
                    f"eval_test/{ef}_new": m["new"],
                }, step=epoch)
        ...
        if old_acc_test > best_test_acc_lab:
            ...
            # W&B: track best epoch + accs (primary + per-eval-func)
            _wandb_safe_log(args, {
                "best/epoch": int(best_epoch),
                "best/primary_all": float(best_stats[0]),
                "best/primary_old": float(best_stats[1]),
                "best/primary_new": float(best_stats[2]),
            }, step=epoch)
            if 'accs_test' in locals():
                for ef, m in accs_test.items():
                    _wandb_safe_log(args, {
                        f"best/{ef}_all": m["all"],
                        f"best/{ef}_old": m["old"],
                        f"best/{ef}_new": m["new"],
                    }, step=epoch)
    ...
    _wandb_safe_log(args, {
        ...
        "final/primary_all": float(best_stats[0]),
        "final/primary_old": float(best_stats[1]),
        "final/primary_new": float(best_stats[2]),
    }, step=best_epoch)
...
def extract_labeled_protos(model, train_loader, args):
    ...
    accs_by_eval = _compute_accs_for_eval_funcs(
        y_true=u_targets.cpu().numpy(),
        y_pred=preds,
        mask=mask_cls,
        args=args,
    ...
    primary_eval = args.eval_funcs[0] if len(getattr(args, "eval_funcs", [])) > 0 else "v1"
    metrics["accs_by_eval"] = accs_by_eval
    metrics["all_acc"] = accs_by_eval.get(primary_eval, next(iter(accs_by_eval.values())))["all"]
    metrics["old_acc"] = accs_by_eval.get(primary_eval, next(iter(accs_by_eval.values())))["old"]
    metrics["new_acc"] = accs_by_eval.get(primary_eval, next(iter(accs_by_eval.values())))["new"]
    prototype_higher=[]
...
def test_kmeans(model, test_loader, epoch,
                 save_name, args,projection_head=None, Use_GPU=False):#TLY Use_GPU=True
    ...
    primary_eval = args.eval_funcs[0] if len(getattr(args, "eval_funcs", [])) > 0 else "v1"
    primary = accs_by_eval.get(primary_eval, next(iter(accs_by_eval.values())))
    return primary["all"], primary["old"], primary["new"], accs_by_eval
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_hyperedges', default=8, type=int)
    parser.add_argument('--num_heads', default=4, type=int)
    parser.add_argument('--topk_tokens_per_edge', default=128, type=int)
    parser.add_argument('--use_gru_update', type=str2bool, default=True)
    parser.add_argument('--use_mmkd', type=str2bool, default=False)
    ...
    parser.add_argument('--use_wandb', action='store_true', default=False,
                        help='Enable Weights & Biases logging (online by default; requires WANDB_API_KEY or wandb login).')
    parser.add_argument('--wandb_entity', default=None, type=str,
                        help='W&B entity/team (optional). If None, uses your default after login; can also run anonymous if supported.')
    parser.add_argument('--wandb_project_prefix', default='GCD', type=str,
                        help='Project name prefix. Full project name will be: <prefix>+<dataset_name>.')
    parser.add_argument('--wandb_run_name', default=None, type=str,
                        help='Run name (optional). If not set, a name like bs{batch_size}_lr{lr}_seed{seed} will be used.')
    parser.add_argument('--wandb_mode', default=None, type=str,
                        choices=[None, 'online', 'offline', 'disabled'],
                        help="Override W&B mode. If set, will export WANDB_MODE accordingly. "
                             "online=upload, offline=local only, disabled=turn off wandb completely.")
    parser.add_argument('--wandb_tags', nargs='*', default=None,
                        help='Optional list of W&B tags.')
    parser.add_argument('--wandb_notes', default=None, type=str,
                        help='Optional notes/description for the W&B run.')
    ...
    # W&B INIT (optional)
    # ----------------------
    if getattr(args, 'use_wandb', False) and (wandb is not None) and (getattr(args, 'wandb_mode', None) != 'disabled'):
        # Respect explicit mode override (online/offline/disabled)
        if getattr(args, 'wandb_mode', None) is not None:
            os.environ['WANDB_MODE'] = str(args.wandb_mode)

        project_name = f"{getattr(args, 'wandb_project_prefix', 'GCD')}+{args.dataset_name}+{args.mode}"
        if args.wandb_run_name is None or len(str(args.wandb_run_name)) == 0:
            # Example: bs128_lr0.1_seed1
            args.wandb_run_name = f"bs{args.batch_size}_lr{args.lr}_seed{args.seed}"

        # If you haven't logged in, W&B may require WANDB_API_KEY / `wandb login`.
        # Some setups allow anonymous logging; we enable it when no entity is provided.
        anon = "allow" if (getattr(args, 'wandb_entity', None) in [None, ""]) else None
        ...
        try:
            wandb.init(
                ...
                anonymous=anon,
                config={k: (v if isinstance(v, (int, float, str, bool, list, dict, type(None))) else str(v))
            ...
            print("[wandb] Continuing without online logging. Tip: run `wandb login` or export WANDB_API_KEY.")
    # ----------------------
    # BASE MODEL
    # ----------------------
    if args.base_model == 'vit_dino':
        ...
        if dino_v1:
            ...
            pretrain_path = mmkd_pretrain_path

            model = create_model(
                model_name='ViT-B-16-notext-quickgelu',
                pretrained=pretrain_path,
                clip_model="CLIPnoText",
                force_quick_gelu=True,
            )

            args.embed_dim = 768
            neck = SCAN(embed_dim=args.embed_dim, num_evidence_slots=args.num_hyperedges,\
                        topk_tokens_per_slot=args.topk_tokens_per_edge, use_gru_update=args.use_gru_update)
            model = CLIPnoTextWithNeck(model, neck)
